...
:monkey_face: 我想要一个尽可能简单好看，并且博客相关所有代码自己编写真正属于自己的博客

:monkey_face: 我想用 ES6 来写我的代码，我想用组件化的方式来组织我的页面结构

:monkey_face: 我不会 Ruby 不会 Go

:monkey_face: 我想实现喜欢的博客样式而不是只能用其他人的主题

:monkey_face: Hexo 的自定义主题也尝试过，由于有一些限制和使用 ejs 之类的 Dynamic Web 不是我想要的

:monkey_face: 我觉得用 markdown 写文章很酷，文章即数据，数据即文件

:monkey_face: 如果自己博客不用，几乎很少改动立马就可以直接放笔记软件或者简书那样支持 markdown 的网站

:monkey_face: 再好的富文本编辑器只能解决丰富的文本编辑需求，却无法给你带来纯字符编辑的乐趣

:monkey_face: 我想要了解 Next.js

:open_hands: 虽然我很努力的在"编"各种理由，但上面一条是认真的 
...
希望大家喜欢并且使用，但是不接受 pull request 如果你对该项目感兴趣并且有任何新的想法和需求可以自行 fork 之后自由发挥 
...
需要注意的是，当你在 `_post` 目录下做如下这些操作之后，刷新浏览器可能会遇到 `404`，原因是分类和文章的唯一标识是依赖于它们的文件名生成的短 hash，而它是页面路由组成的一部分，路由映射关系详见 Next.js 关于 [`next.config.js`](https://nextjs.org/docs/#static-html-export) 的说明，当改变了文件名，数据文件已经更新，但是你正在浏览的页面 URL 可能没有改变，所以导致了 `404` 不过只是编辑 markdown 文件内容则完全没有影响，并且导出命令导出的静态页面不会出问题，只是开发模式会有这个问题
